...
dirMap.set(5, `量子少年/${moment().format('YYYY.MM')}`);
dirMap.set(6, `量子少年/${moment().format('YYYY.MM')}`);
dirMap.set(18, `量子少年/${moment().format('YYYY.MM')}`);
dirMap.set(19, `量子少年/${moment().format('YYYY.MM')}`);
dirMap.set(9, `EOE组合/${moment().format('YYYY.MM')}`);
dirMap.set(10, `EOE组合/${moment().format('YYYY.MM')}`);
dirMap.set(11, `EOE组合/${moment().format('YYYY.MM')}`);
dirMap.set(12, `EOE组合/${moment().format('YYYY.MM')}`);
dirMap.set(13, `EOE组合/${moment().format('YYYY.MM')}`);
dirMap.set(14, `sp9/明前奶绿/${moment().format('YYYY.MM')}`);
dirMap.set(15, `星律动/${moment().format('YYYY.MM')}`);
dirMap.set(16, `星律动/${moment().format('YYYY.MM')}`);
dirMap.set(17, `星律动/${moment().format('YYYY.MM')}`);
dirMap.set(20, `sp7/麻尤米mayumi/${moment().format('YYYY.MM')}`);
dirMap.set(21, `sp9/凜凜蝶凜/${moment().format('YYYY.MM')}`);
        ...
        for (let i = 0; i < users.length; ++i) {
            ...
            if (dirMap.has(user.id)) {
                ...
                if (prefixSet.has(user.id)) {
                    clip.playUrl = `rec.bili.studio/d/${dirMap.get(user.id)}/${liveTime.format('YYYYMMDD-HHmmss')}-${user.name}-${clip.title}.mp4`;
                    clip.redirectUrl = `rec.bili.studio/${dirMap.get(user.id)}/${liveTime.format('YYYYMMDD-HHmmss')}-${user.name}-${clip.title}.mp4`;
                } else {
                    clip.playUrl = `rec.bili.studio/d/${dirMap.get(user.id)}/${liveTime.format('YYYYMMDD')}-${user.name}-${clip.title}.mp4`;
                    clip.redirectUrl = `rec.bili.studio/${dirMap.get(user.id)}/${liveTime.format('YYYYMMDD')}-${user.name}-${clip.title}.mp4`;    
                }
            }
            console.log(clip);
